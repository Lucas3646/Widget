...
struct PriceWidgetProvider: AppIntentTimelineProvider {
    func placeholder(in context: Context) -> PriceWidgetEntry {
        PriceWidgetEntry(
            date: Date(),
            symbol: "^NDX",
            quote: QuoteSnapshot(symbol: "^NDX", name: "NASDAQ 100", price: 23_500.00, previousClose: 23_300.00, changePercent: 0.86, updatedAt: Date())
        )
    }
    ...
    func snapshot(for configuration: AssetWidgetIntent, in context: Context) async -> PriceWidgetEntry {
        ...
        let quote = try? await MarketService.quote(symbol: symbol)
        return PriceWidgetEntry(date: Date(), symbol: symbol, quote: quote)
    }

    func timeline(for configuration: AssetWidgetIntent, in context: Context) async -> Timeline<PriceWidgetEntry> {
        let symbol = normalized(configuration.symbol)
        let quote = try? await MarketService.quote(symbol: symbol)
        let entry = PriceWidgetEntry(date: Date(), symbol: symbol, quote: quote)
        ...
        VStack(alignment: .leading, spacing: 6) {
            HStack(alignment: .firstTextBaseline) {
                VStack(alignment: .leading, spacing: 1) {
                    Text(entry.quote?.name ?? entry.symbol)
            ...
            if let quote = entry.quote {
                ...
                HStack(spacing: 8) {
                    ...
                    if let previous = quote.previousClose {
                        let absolute = quote.price - previous
                        Text(String(format: "%+.2f", absolute))
                            .font(.caption)
                            .foregroundStyle(muted)
                    }
                }
...
struct MarketPriceWidget: Widget {
    ...
    var body: some WidgetConfiguration {
        AppIntentConfiguration(kind: kind, intent: AssetWidgetIntent.self, provider: PriceWidgetProvider()) { entry in
            PriceWidgetView(entry: entry)
        }
        .configurationDisplayName("Cours marché")
        .description("Cours actuel et variation de l'actif choisi.")
