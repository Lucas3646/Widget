...
/** Reads IBKR's own Time Weighted Return from ChangeInNAV. TWR removes the effect of deposits/withdrawals. */
object IbkrTwrRepository {
    ...
    fun refresh(context:Context,timeframe:PortfolioTimeframe):Double?{
        ...
        val token=URLEncoder.encode(c.token,"UTF-8");val query=URLEncoder.encode(c.queryId,"UTF-8")
        val today=LocalDate.now();val start=when(timeframe){
        ...
        var body=""
        repeat(6){if(it>0)Thread.sleep(1000);body=get("$BASE/GetStatement?t=$token&q=${URLEncoder.encode(ref,"UTF-8")}&v=3");if(body.contains("<FlexQueryResponse"))return@repeat}
        // ChangeInNAV is the account-level row. IBKR reports twr as a percentage for the requested statement period.
        val element=Regex("<ChangeInNAV\\b[^>]*>").find(body)?.value ?: Regex("<ChangeInNAV\\b[^>]*/>").find(body)?.value ?: return null
    ...
    private fun get(url:String):String{
        val c=(URL(url).openConnection() as HttpURLConnection).apply{connectTimeout=12000;readTimeout=20000;setRequestProperty("User-Agent","MarketWidgets/1.8 Android")}
